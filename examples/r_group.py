...
# The Gelman-Rubin R value can be used to determine whether N chains explored the
# same mode. Pypmc offers a function which groups chains with a common R value
# less than some ``critical_r``. In this example, we run five Markov Chains
# initialized in different modes and then group chains which explored same mode.
...
# Here we place two chain into the mode at [5, 0.01] and three into the mode at [-4,1].
...
# R value calculation only needs the means, variances (diagonal elements of covariance matrix) and number of samples
# for simpler usage, r_group takes the full covariance matrix
...
found_groups = pypmc.mix_adapt.r_value.r_group([np.mean(chain,   axis=0) for chain in stacked_values],
                                               [np.cov (chain, rowvar=0) for chain in stacked_values],
...
pypmc.mix_adapt.r_value.make_r_gaussmix(stacked_values)
